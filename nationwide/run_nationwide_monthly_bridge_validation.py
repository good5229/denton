...
INDEX_PATHS = {
    ...
    "service": ROOT / "data" / "processed" / "rolling_service_production_index.csv",
...
SERVICE_ACTIVITY_MAP = {
    ...
    "금융 및 보험업": {"금융 및 보험업"},
    "부동산업": {"부동산업"},
    "사업서비스업": {"전문 과학 및 기술 서비스업", "사업시설 관리 사업 지원 및 임대 서비스업"},
    "공공 행정, 국방·사회보장": {"공공행정 국방 및 사회보장 행정"},
    "교육 서비스업": {"교육 서비스업"},
...
def build_monthly_weights(q: pd.DataFrame) -> pd.DataFrame:
    ...
    # 서비스 세부업종: only use if the local source is truly monthly.
    # The rolling service file currently held locally is quarterly-coded
    # (prd_se=Q, prd_de=YYYY01~YYYY04), so it is intentionally excluded by
    # load_index().  If a monthly service source is collected later, this block
    # will automatically become active.
    service = load_index(INDEX_PATHS["service"])
    frames = []
    ...
    if frames:
        ...
        # within region-month after selecting their public subindices.
        svc = (
            svc.groupby(["region_short", "activity_group", "year", "quarter", "month", "period"], as_index=False)
            .agg(indicator_value=("indicator_value", "mean"))
            .assign(monthly_indicator_source="시도별 서비스업생산지수")
        )
