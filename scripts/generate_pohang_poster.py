...
def main() -> Path:
    ...
    for xx0, num, title, frame, color, footer in [(M, "08", "공표 후 정밀화: 격차 작은 중분류", precise_frame, TEAL, "속보: 월 변화 경보 · 정밀화: 공표 후 재산출"), (x2, "09", "공표 후 정밀화: 금액격차 큰 중분류", gap_frame, RED, "10% 초과 산업은 주의·자료보강")]:
        ...
        native_table(slide, x, y, cw, ["중분류", "실제", "Q+1M\n속보", "정밀화", "속보오차", "정밀오차"], rows, [.30, .13, .13, .13, .155, .155], 62, [12, 12, 12, 12, 10, 10])
        desc = "단위: 억원. 속보는 상위산업 1~4분기+1개월 총량과 과거 중분류 구조를 결합한 값이다." if color == TEAL else "정확성 개선 후에도 남은 금액격차다. 속보 단계에서는 주의·자료보강으로 표시하고 공표 후 직접 활동자료로 재산출한다."
        textbox(slide, x, y + 445, cw, 115, desc, 16, MUTED, False, "center")
        rows = [("속보성", "공표 전 월 변화·경보"), ("정밀화", "공표 후 실제-추정 격차"), ("사업장 구조", "고용·산재보험 자료 무악화 적용")]
    ...
    conclusion_cards = [("방법론 기여", ["공식 GVA를 읍면동×월×산업으로 전환", "전 산업 19대·74중·228소분류 동시 산출", "속보성 지표와 정확성 개선 지표 분리", "공간·시간·산업 총량 제약 보존", "중분류 실제값과 비교해 오차 위치 식별", "고양·포항 공통 구조로 확장성 확인"]), ("검증 기여", ["속보 단계: 공표 전 월 변화·경보", "정밀화 단계: 공표 후 GVA 격차 검증", "정밀 총오차 9,066억→7,718억", "사업장 구조자료 적용 후 악화 셀 0개", "억원·상대오차를 함께 표기", "주의·자료보강 산업 분리"]), ("정책 기여", ["29개 읍면동 산업활력 격차 지도화", "월 변화로 조기경보 후보 선별", "산단·항만·상권·고용정책 우선순위 연결", "유료 카드자료 없이 무료 자료 기반 갱신", "10% 초과 산업은 자료보강 대상으로 분리", "공모전 평가요소: 정확성·실현성·공공성 대응"])]
    card_w = (cw - 36) / 3
