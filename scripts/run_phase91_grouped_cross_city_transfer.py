#!/usr/bin/env python3
"""Phase91: grouped cross-city transfer screen for middle-industry GVA errors.

Phase90 identified all weak/worsened middle-industry cells and grouped them by
error mechanism.  Phase91 tests whether the over/under-estimation pattern
...
the selected group's queue error and does not increase whole-city WAPE.
"""
...
OUTDIR = DATA / "phase91_grouped_cross_city_transfer"
REPORT = ROOT / "reports" / "partial_statistics_estimation_phase91_grouped_cross_city_transfer.md"
            ...
            decisions.append(
                {
                    ...
                    "reason": "WAPE·10%초과오차·10%이하 셀 수를 동시에 통과한 후보 없음",
                }
            )
            continue
        ...
        decisions.append(
            {
                ...
                "reason": "그룹 큐오차·10%초과오차 감소 및 도시 전체 지표 비악화",
            }
        )
...
def main() -> None:
    ...
    final.to_csv(OUTDIR / "phase91_grouped_transfer_final_detail.csv", index=False, encoding="utf-8-sig")
    decisions.to_csv(OUTDIR / "phase91_group_decisions.csv", index=False, encoding="utf-8-sig")
    candidates.to_csv(OUTDIR / "phase91_candidate_screen.csv", index=False, encoding="utf-8-sig")
    summary.to_csv(OUTDIR / "phase91_summary_by_city.csv", index=False, encoding="utf-8-sig")
    qwide.to_csv(OUTDIR / "phase91_queue_improvement_summary.csv", index=False, encoding="utf-8-sig")
...
{md_table(wide, [("city", "지역"), ("error_sum_eok_baseline", "기준 오차 억원"), ("error_sum_eok_phase91", "Phase91 오차 억원"), ("error_reduction_eok", "감소 억원"), ("wape_pct_baseline", "기준 WAPE %"), ("wape_pct_phase91", "Phase91 WAPE %"), ("wape_improvement_pp", "개선 pp"), ("within_10pct_cells_baseline", "기준 10%이하"), ("within_10pct_cells_phase91", "Phase91 10%이하"), ("over_20pct_cells_baseline", "기준 20%초과"), ("over_20pct_cells_phase91", "Phase91 20%초과")])}

## Phase90 큐 개선

{md_table(qwide, [("city", "지역"), ("queue_cells_baseline", "큐 셀 개수"), ("queue_error_sum_eok_baseline", "기준 큐오차 억원"), ("queue_error_sum_eok_phase91", "Phase91 큐오차 억원"), ("queue_error_reduction_eok", "큐오차 감소 억원"), ("queue_gap_to_10pct_eok_baseline", "기준 10%초과 억원"), ("queue_gap_to_10pct_eok_phase91", "Phase91 10%초과 억원"), ("queue_gap10_reduction_eok", "10%초과 감소 억원"), ("queue_within_10pct_cells_baseline", "기준 10%이하"), ("queue_within_10pct_cells_phase91", "Phase91 10%이하")])}
...
"""
    ...
    print(REPORT)
    print(OUTDIR / "phase91_grouped_transfer_final_detail.csv")
