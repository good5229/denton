...
def main() -> int:
    ...
    require(final["official_quarterly_target_materialized"] is False, "target cube should remain extraction-pending")
    ...
    require(source["target_extraction_gate"].eq("blocked_pdf_table_parser_not_implemented").all(), "target extraction gate mismatch")

    official_cube = pd.read_parquet(PROCESSED_DIR / "partial_stats_phase22_gva_official_target_cube.parquet")
    require(len(official_cube) == 0, "official target cube must not be fabricated without parser")
