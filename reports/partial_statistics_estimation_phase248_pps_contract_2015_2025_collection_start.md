...
- 2016-10-01 1페이지 단독 smoke 요청도 `HTTP 429`로 실패했으므로, 현재는 기간 단위가 아니라 계정/IP/API 게이트웨이 단위 제한이 남아 있는 상태로 판단한다.
- 중단 과정에서 manifest에 6자리 YYYYMM이 아닌 깨진 period 행이 1개 남은 것을 제거했고, 수집기와 phase249 품질감사는 invalid period 행을 보존하지 않도록 보강했다.
