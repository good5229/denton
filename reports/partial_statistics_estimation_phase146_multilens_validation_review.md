...
확인된 운영 성능은 2022~2023 두 개 holdout 연도에서, 분기 rolling 시점에 생성한 **연간 중분류 GVA 예측**을 사후 연간 actual과 비교한 집계검증 기준이다. 고양시는 Q1+1개월 WAPE 5.13%, Q1~Q3+1개월 WAPE 1.60%다. 포항시는 Q1+1개월 WAPE 3.91%, Q1~Q3+1개월 WAPE 1.23%다. 분기 자체 GVA의 독립 actual 검증은 아직 제한적이며, Q4+1개월은 연간 통제총량을 회수하는 회계 정산 단계이므로 예측력으로 해석하지 않는다.
...
검증 결과, Q4 연간 회수 및 현재분기+이전기간=누계 정합성은 통과했다. 최대 회계 차이는 0.00000000억원 수준이며, 이는 부동소수점 오차로 볼 수 있다. 이 항목은 예측성능이 아니라 회계 정합성 검증이다.
