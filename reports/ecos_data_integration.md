...
1. 전기·가스 산업 예측 스크립트에 `energy_exogenous_with_ecos_quarterly.csv`를 연결해 기존 FRED-only feature와 성능을 비교한다. 완료: `reports/release_aware_modeling_policy.md`에 정리.
2. 상세산업 배분 스크립트에 `ecos_io_middle_industry_prior.csv`를 선택적 가중치로 결합한다. 완료: `scripts/allocate_detailed_industry.py`에 lag-aware proxy와 ECOS IO prior를 반영.
3. 대시보드의 데이터 설명 또는 메타데이터 패널에 `actual`, `benchmark`, `prior`, `exogenous`의 출처 구분을 표시한다. 완료: `reports/dashboard/index.html`에 역할 구분 패널 추가.
